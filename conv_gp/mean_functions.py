...
class PatchwiseConv2d(Conv2dMean):
    def __init__(self, filter_size, feature_maps, out_height, out_width):
        super().__init__(filter_size, feature_maps)
        self.out_height = out_height
        self.out_width = out_width

    @params_as_tensors
    def __call__(self, PNL_patches):
        kernel = tf.reshape(self.conv_filter, [self.filter_size**2 * self.feature_maps,
            self.feature_maps])
